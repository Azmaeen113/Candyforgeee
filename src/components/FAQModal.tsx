...
const FAQ_DATA = [
  {
    title: "How to earn $ATM tokens",
    content: (
      <div className="space-y-3 text-sm font-[300]">
        <p>To start earning $ATM tokens, follow these steps:</p>
          ...
          <li>
            ...
            <ul className="list-disc pl-5 mt-2 space-y-1">
              <li>
                ...
                purchased in the 'Shop' and increase the rate of $ATM mining.
              </li>
              <li>
  ...
  },
  {
    ...
    content: (
      <p className="text-sm font-[300]">
        Mining in ATM is based on principles similar to classic mining
  ...
  },
  {
    ...
    content: (
      <p className="text-sm font-[300]">
        The listing is scheduled for the beginning of Q2. The ATM team and its
  ...
  {
    title: "Who are the partners and investors of the ATM project?",
    content: (
      <div className="space-y-3 text-sm font-[300]">
        <p>The partners and investors of the ATM project include:</p>
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-50 flex overflow-y-scroll text-white font-poppins"
          style={{ background: 'linear-gradient(180deg, #1A0B2E 0%, #0A0E27 50%, #1A0B2E 100%)' }}
        >
          <div className="flex flex-col h-full p-4 w-full ">
            <button
              onClick={onClose}
              className="absolute right-4 top-4 w-10 h-10 flex items-center justify-center rounded-full transition-all duration-300 hover:scale-110"
              style={{
                background: 'rgba(255, 16, 240, 0.2)',
                border: '1px solid rgba(255, 16, 240, 0.3)'
              }}
            >
              <IoCloseSharp size={24} className="text-[#FF10F0]" />
            </button>
            ...
            <div className="w-full space-y-2 last:pb-28">
              {FAQ_DATA.map((item, index) => (
                <div key={index} className="">
                  <button
                    className="flex justify-between items-center w-full pt-3 pb-2 text-left font-medium rounded-lg px-2 transition-all duration-200 outline-none"
                    onClick={() => toggleAccordion(index)}
                    style={{
                      background: activeIndex === index ? 'rgba(0, 245, 255, 0.1)' : 'transparent'
                    }}
                  >
                  ...
                  >
                    <AnimatePresence>
                      {activeIndex === index && (
                        ...
                        >
                          <div className="pb-4 text-gray-300">{item.content}</div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
